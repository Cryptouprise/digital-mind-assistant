...
const ZoomMeeting: React.FC<ZoomMeetingProps> = ({ 
  ...
  const joinZoomMeeting = () => {
    try {
      ...
      // In a production environment, you would generate this server-side
      // For testing, you need to provide your API Key and generate a signature
      // The signature generation requires your API Secret which should NEVER be in client-side code
      const apiKey = 'YOUR_ZOOM_API_KEY'; // Replace with your Zoom API Key
      const meetingPassword = ''; // Optional: Add password if meeting requires it
      
      // TODO: In production, get signature from your backend
      // For demo: This should be generated by a server to avoid exposing the API Secret
      const signature = 'YOUR_SIGNATURE'; // Replace with signature from your backend
  ...
  const toggleVideo = () => {
    if (!window.ZoomMtg?.videoOff) return;
